...
const BaseTypeMasterList = () => {
  ...
  const getAllUsers = async (page, perpage) => {
    ...
    await fetch(page ? `${GET_TYPE}?page=${page}` : GET_TYPE, {
      method: "POST",
      headers: {
  ...
  const updateStatus = async (id, status) => {
    ...
    const StoredData = localStorage.getItem("authUser");
    const Token = JSON.parse(StoredData);
    await fetch(UPDATE_TYPE_STATUS + id, {
      method: "POST",
      headers: {
        Authorization: "Bearer " + Token.access_token,
        Accept: "application/json",
    ...
    return (
      <>
        <tr>
          <th>Action</th>
          <th>Name</th>
          <th>Install Base Make Name</th>
          <th>Install Base Modal Name</th>
    ...
    return (
      <>
        <td>
          <>
            <AiTwotoneEdit
              ...
              onClick={() =>
                history.push(
                  process.env.PUBLIC_URL + "/edit-install-base-type/" + item.id,
        ...
        <td>
          ...
          <p className="mb-0">
            <span className="fw-medium">{`${item.install_type}`} </span>
          </p>
        </td>
        <td>
          <h5 className="font-size-14 text-truncate"></h5>
        ...
        <td>
          <h5 className="font-size-14 text-truncate"></h5>
          <p className="mb-0">
            <span className="fw-medium">{`${item.model_name}`} </span>
          </p>
        </td>
          ...
          {isLoading ? (
            ...
            <>
              <TableComponent
                ...
                btnName={"Add Type"}
                addData={"/create-install-base-type"}
                editData={"/edituser/"}
                viewData={"/viewuser/"}
                SearchData={["install_type"]}
                isDeleteUser={false}
              />
            </>
          )}
...
export default BaseTypeMasterList;
