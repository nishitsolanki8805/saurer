...
const FiltrationMakeList = () => {
  ...
  const [breadcrumbItems] = useState([
    ...
    {
      title: "Install Filtration Make List",
      link: "#",
    },
  ]);
  ...
  const getAllUsers = async (page, perpage) => {
    ...
    await fetch(
      page ? `${GET_FILTRATION_MAKE}?page=${page}` : GET_FILTRATION_MAKE,
      {
        method: "POST",
        headers: {
  ...
  const updateStatus = async (id, status) => {
    ...
    const StoredData = localStorage.getItem("authUser");
    const Token = JSON.parse(StoredData);
    await fetch(UPDATE_FILTRATION_MAKE_STATUS + id, {
      method: "POST",
      headers: {
        Authorization: "Bearer " + Token.access_token,
        Accept: "application/json",
    ...
    return (
      <>
        <td>
          <>
            <AiTwotoneEdit
              ...
              onClick={() =>
                history.push(
                  process.env.PUBLIC_URL + "/edit-filtration-make/" + item.id,
        ...
        <td>
          ...
          <p className="mb-0">
            <span className="fw-medium">{`${item.filtration_make_name}`} </span>
          </p>
        </td>
        <td>
          <h5 className="font-size-14 text-truncate"></h5>
          <p className="mb-0">
            <span className="fw-medium">
              {`${item.filtration_make_company_name}`}{" "}
            </span>
          </p>
        </td>
          ...
          {isLoading ? (
            ...
            <>
              <TableComponent
                ...
                btnName={"Add Filtration Make"}
                addData={"/create-filtration-make"}
                editData={"/edituser/"}
                viewData={"/viewuser/"}
                SearchData={["filtration_make_name"]}
                isDeleteUser={false}
              />
            </>
          )}
...
export default FiltrationMakeList;
