...
import {
  ...
  GET_DEPARTMENT,
  GET_PROCESS_LINE,
  GET_PRODUCTION_ENDUSE,
  UPDATE_DEPARTMENT_STATUS,
  UPDATE_PROCESS_LINE_STATUS,
  UPDATE_PRODUCTION_ENDUSE_STATUS,
} from "../../global";
import Switch from "react-switch";
import TableComponent from "../../components/Common/TableComponent";

const ProductionEnduseList = () => {
  ...
  const getAllUsers = async (page, perpage) => {
    ...
    await fetch(
      page ? `${GET_PRODUCTION_ENDUSE}?page=${page}` : GET_PRODUCTION_ENDUSE,
      {
        method: "POST",
        headers: {
  ...
  const updateStatus = async (id, status) => {
    ...
    const StoredData = localStorage.getItem("authUser");
    const Token = JSON.parse(StoredData);
    await fetch(UPDATE_PRODUCTION_ENDUSE_STATUS + id, {
      method: "POST",
      headers: {
        Authorization: "Bearer " + Token.access_token,
        Accept: "application/json",
    ...
    return (
      <>
        <td>
          <>
            <AiTwotoneEdit
              ...
              onClick={() =>
                history.push(
                  process.env.PUBLIC_URL + "/edit-production-enduse/" + item.id,
        ...
        <td>
          ...
          <p className="mb-0">
            <span className="fw-medium">{`${item.production_name}`} </span>
          </p>
        </td>
          ...
          {isLoading ? (
            ...
            <>
              <TableComponent
                ...
                btnName={"Add Production Enduse"}
                addData={"/create-production-enduse"}
                editData={"/edituser/"}
                viewData={"/viewuser/"}
                SearchData={["production_name"]}
                isDeleteUser={false}
              />
            </>
          )}
...
export default ProductionEnduseList;
