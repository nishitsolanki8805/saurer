...
const DepartmentList = () => {
  ...
  const getAllUsers = async (page, perpage) => {
    ...
    await fetch(page ? `${GET_DEPARTMENT}?page=${page}` : GET_DEPARTMENT, {
      method: "POST",
      headers: {
  ...
  const updateStatus = async (id, status) => {
    ...
    const StoredData = localStorage.getItem("authUser");
    const Token = JSON.parse(StoredData);
    await fetch(UPDATE_DEPARTMENT_STATUS + id, {
      method: "POST",
      headers: {
        Authorization: "Bearer " + Token.access_token,
        Accept: "application/json",
        ...
        <td>
          <>
            <AiTwotoneEdit
              ...
              onClick={() =>
                history.push(
                  process.env.PUBLIC_URL + "/edit-department/" + item.id,
          ...
          {isLoading ? (
            ...
            <>
              <TableComponent
                ...
                addData={"/create-department"}
                editData={"/edituser/"}
                viewData={"/viewuser/"}
                SearchData={["department_name"]}
                isDeleteUser={false}
              />
            </>
          )}
