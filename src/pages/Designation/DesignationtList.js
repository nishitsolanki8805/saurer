...
const DesignationtList = () => {
  ...
  const getAllUsers = async (page, perpage) => {
    ...
    await fetch(page ? `${GET_DESIGNATION}?page=${page}` : GET_DESIGNATION, {
      method: "POST",
      headers: {
  ...
  const updateStatus = async (id, status) => {
    ...
    const StoredData = localStorage.getItem("authUser");
    const Token = JSON.parse(StoredData);
    await fetch(UPDATE_DESIGNATION_STATUS+ id, {
      method: "POST",
      headers: {
        Authorization: "Bearer " + Token.access_token,
        Accept: "application/json",
        ...
        <td>
          <>
            <AiTwotoneEdit
              ...
              onClick={() =>
                history.push(
                  process.env.PUBLIC_URL + "/edit-designation/" + item.id,
          ...
          {isLoading ? (
            ...
            <>
              <TableComponent
                ...
                addData={"/create-designation"}
                editData={"/edituser/"}
                viewData={"/viewuser/"}
                SearchData={["designation_name"]}
                isDeleteUser={false}
              />
            </>
          )}
...
export default DesignationtList;
