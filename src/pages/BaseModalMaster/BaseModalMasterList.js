...
import { GET_MODAL, UPDATE_MODAL_STATUS } from "../../global";
import Switch from "react-switch";
import TableComponent from "../../components/Common/TableComponent";

const BaseModalMasterList = () => {
  ...
  const getAllUsers = async (page, perpage) => {
    ...
    await fetch(page ? `${GET_MODAL}?page=${page}` : GET_MODAL, {
      method: "POST",
      headers: {
  ...
  const updateStatus = async (id, status) => {
    ...
    const StoredData = localStorage.getItem("authUser");
    const Token = JSON.parse(StoredData);
    await fetch(UPDATE_MODAL_STATUS + id, {
      method: "POST",
      headers: {
        Authorization: "Bearer " + Token.access_token,
        Accept: "application/json",
          ...
          {isLoading ? (
            ...
            <>
              <TableComponent
                ...
                btnName={"Add Modal"}
                addData={"/create-install-base-modal"}
                editData={"/edituser/"}
                viewData={"/viewuser/"}
                SearchData={["make_name", "model_name"]}
                isDeleteUser={false}
              />
            </>
          )}
...
export default BaseModalMasterList;
