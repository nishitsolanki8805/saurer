...
const ProcessLineList = () => {
  ...
  const getAllUsers = async (page, perpage) => {
    ...
    await fetch(page ? `${GET_PROCESS_LINE}?page=${page}` : GET_PROCESS_LINE, {
      method: "POST",
      headers: {
  ...
  const updateStatus = async (id, status) => {
    ...
    const StoredData = localStorage.getItem("authUser");
    const Token = JSON.parse(StoredData);
    await fetch(UPDATE_PROCESS_LINE_STATUS + id, {
      method: "POST",
      headers: {
        Authorization: "Bearer " + Token.access_token,
        Accept: "application/json",
    ...
    return (
      <>
        <td>
          <>
            <AiTwotoneEdit
              ...
              onClick={() =>
                history.push(
                  process.env.PUBLIC_URL + "/edit-process-line/" + item.id,
        ...
        <td>
          ...
          <p className="mb-0">
            <span className="fw-medium">{`${item.process_line_name}`} </span>
          </p>
        </td>
          ...
          {isLoading ? (
            ...
            <>
              <TableComponent
                ...
                btnName={"Add Process Line"}
                addData={"/create-process-line"}
                editData={"/edituser/"}
                viewData={"/viewuser/"}
                SearchData={["process_line_name"]}
                isDeleteUser={false}
              />
            </>
          )}
...
export default ProcessLineList;
