...
class CreateProcessLine extends Component {
  constructor(props) {
    super(props);
    this.state = {
      breadcrumbItems: [
        ...
        {
          title: "Process Line List",
          link: process.env.PUBLIC_URL + "/process-line-list",
        },
        {
    ...
    this.validationSchema = this.props.match.params.id
      ? Yup.object().shape({
          process_line_name: Yup.string().required("Enter Process Line Name"),
        })
      : Yup.object().shape({
          process_line_name: Yup.string().required("Enter Process Line Name"),
        });
  ...
  async GetUserById(id) {
    ...
    await fetch(GET_PROCESS_LINE_BY_ID + id, {
      method: "GET",
      headers: {
  ...
  createUser = async (data) => {
    ...
    const Token = JSON.parse(StoredData);
    try {
      const response = await fetch(CREATE_PROCESS_LINE, {
        method: "POST",
        headers: {
          Authorization: "Bearer " + Token.access_token,
          Accept: "application/json",
  ...
  UpdateUser = async (data) => {
    ...
    try {
      const response = await fetch(
        UPDATE_PROCESS_LINE + this.state.dataByID.id,
        {
          method: "POST",
          headers: {
            Authorization: "Bearer " + Token.access_token,
              ...
              <Card>
                <CardBody>
                  <Formik
                    ...
                    initialValues={{
                      process_line_name: this.state.isEdit
                        ? this.state.dataByID.process_line_name
                        : "",
                    }}
                    validationSchema={this.validationSchema}
                    onSubmit={(values, { resetForm }) => {
                      console.log("process_line_name", values);
                              ...
                              <div className="field">
                                ...
                                <div className="control">
                                  <Field
                                    ...
                                    value={values.process_line_name}
                                    type="text"
                                    className={`form-control ${
                                      errors.process_line_name &&
                                      touched.process_line_name
                                        ? "is-invalid"
                                        : ""
                                    }`}
                                    placeholder="Enter Process Line Name"
                                    error={
                                      errors.process_line_name &&
                                      touched.process_line_name
                                    }
...
export default CreateProcessLine;
