...
const CreateMakeList = () => {
  ...
  const getAllUsers = async (page, perpage) => {
    ...
    await fetch(page ? `${GET_MAKE}?page=${page}` : GET_MAKE, {
      method: "POST",
      headers: {
  ...
  const updateStatus = async (id, status) => {
    ...
    const StoredData = localStorage.getItem("authUser");
    const Token = JSON.parse(StoredData);
    await fetch(UPDATE_MAKE_STATUS + id, {
      method: "POST",
      headers: {
        Authorization: "Bearer " + Token.access_token,
        Accept: "application/json",
    ...
    return (
      <>
        <td>
          <>
            <AiTwotoneEdit
              ...
              onClick={() =>
                history.push(
                  process.env.PUBLIC_URL + "/edit-install-base-make/" + item.id,
        ...
        <td>
          <h5 className="font-size-14 text-truncate"></h5>
          <p className="mb-0">
            <span className="fw-medium">{`${item.make_company_name}`} </span>
          </p>
        </td>
          ...
          {isLoading ? (
            ...
            <>
              <TableComponent
                ...
                btnName={"Add Make"}
                addData={"/create-install-base-make"}
                editData={"/edituser/"}
                viewData={"/viewuser/"}
                SearchData={["make_name"]}
                isDeleteUser={false}
              />
            </>
          )}
