...
import {
  ...
  UPDATE_MAKE_STATUS,
  UPDATE_PNEUMATIC_MAKE_STATUS,
} from "../../global";
import Switch from "react-switch";
import TableComponent from "../../components/Common/TableComponent";

const PneumaticMakeList = () => {
  ...
  const getAllUsers = async (page, perpage) => {
    ...
    await fetch(
      page ? `${GET_PNEUMATIC_MAKE}?page=${page}` : GET_PNEUMATIC_MAKE,
      {
        method: "POST",
        headers: {
  ...
  const updateStatus = async (id, status) => {
    ...
    const StoredData = localStorage.getItem("authUser");
    const Token = JSON.parse(StoredData);
    await fetch(UPDATE_PNEUMATIC_MAKE_STATUS + id, {
      method: "POST",
      headers: {
        Authorization: "Bearer " + Token.access_token,
        Accept: "application/json",
    ...
    return (
      <>
        <td>
          <>
            <AiTwotoneEdit
              ...
              onClick={() =>
                history.push(
                  process.env.PUBLIC_URL + "/edit-pneumatic-make/" + item.id,
        ...
        <td>
          ...
          <p className="mb-0">
            <span className="fw-medium">{`${item.pneumatic_make_name}`} </span>
          </p>
        </td>
        <td>
          <h5 className="font-size-14 text-truncate"></h5>
          <p className="mb-0">
            <span className="fw-medium">
              {`${item.pneumatic_make_company_name}`}{" "}
            </span>
          </p>
        </td>
          ...
          {isLoading ? (
            ...
            <>
              <TableComponent
                ...
                btnName={"Add Pneumatic Make"}
                addData={"/create-pneumatic-make"}
                editData={"/edituser/"}
                viewData={"/viewuser/"}
                SearchData={["pneumatic_make_name"]}
                isDeleteUser={false}
              />
            </>
          )}
...
export default PneumaticMakeList;
